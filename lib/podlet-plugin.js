...
const podiumPodletFastifyPlugin = async (
    fastify,
    { podlet, logger, devTool = { enabled: false, port: 0 } },
    done,
) => {
    ...
    if (podlet.development) {
        this.devTool = new DevTool({
            enabled: devTool.enabled,
            logger,
            port: devTool.port,
        });
        this.devTool.register(podlet);

        await this.devTool.start();
        fastify.addHook('onClose', async (instance, cb) => {
